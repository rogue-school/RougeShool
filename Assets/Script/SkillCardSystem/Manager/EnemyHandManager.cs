...
namespace Game.CombatSystem.Manager
{
    ...
    public class EnemyHandManager : MonoBehaviour, IEnemyHandManager
    {
        ...
        public void SetHandCards(List<ISkillCard> cards)
        {
            if (cards == null)
            {
                Debug.LogError("[EnemyHandManager] 설정할 카드 리스트가 null입니다.");
                return;
            }

            // 기존 핸드카드 모두 제거 (안전하게)
            StartCoroutine(SafeClearHandAfterAllAnimations());

            // 새 카드들 배치
            for (int i = 0; i < cards.Count && i < 3; i++)
            {
                var card = cards[i];
                var slotPosition = (SkillCardSlotPosition)(i + 4); // ENEMY_SLOT_1, ENEMY_SLOT_2, ENEMY_SLOT_3
            ...
            Debug.Log($"[EnemyHandManager] 핸드카드 설정 완료: {cards.Count}장");
        }

        /// <summary>
        /// 특정 슬롯에 카드를 복원합니다. (저장 시스템용)
        /// </summary>
        /// <param name="card">복원할 카드</param>
        /// <param name="slotPosition">슬롯 위치</param>
        public void RestoreCardToHand(ISkillCard card, SkillCardSlotPosition slotPosition)
        {
            if (card == null)
            {
                Debug.LogError("[EnemyHandManager] 복원할 카드가 null입니다.");
                return;
            }

            if (slotPosition < SkillCardSlotPosition.ENEMY_SLOT_1 || slotPosition > SkillCardSlotPosition.ENEMY_SLOT_3)
            {
                Debug.LogError($"[EnemyHandManager] 잘못된 슬롯 위치입니다: {slotPosition}");
                return;
            }

            // 기존 카드가 있다면 제거
            if (_cardsInSlots.ContainsKey(slotPosition))
            {
                var existingCard = _cardsInSlots[slotPosition].Item1;
            ...
            // 새 카드 배치
            if (handSlots.TryGetValue(slotPosition, out var slot))
            {
                var ui = slot.AttachCard(card, cardUIPrefab);
                if (ui != null)
                {
                    cardUIs[slotPosition] = ui;
                    _cardsInSlots[slotPosition] = (card, ui);
                }
            }
        ...
        /// <summary>
        /// 지정된 슬롯의 카드를 반환합니다. (저장 시스템용)
        /// </summary>
        /// <param name="slotPosition">슬롯 위치</param>
        /// <returns>해당 슬롯의 카드</returns>
        public ISkillCard GetCardInSlot(SkillCardSlotPosition slotPosition)
        {
            if (_cardsInSlots.TryGetValue(slotPosition, out var cardData))
            {
                return cardData.Item1;
            }
            return null;
        }
    }
