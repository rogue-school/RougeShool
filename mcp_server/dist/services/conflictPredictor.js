...
export class ConflictPredictor {
     ...
     * 네이밍 충돌 예측
     */
    async predictNamingConflicts(newCode, targetFile) {
        const conflicts = {
     ...
     * 클래스명 충돌 찾기
     */
    findClassNameConflicts(newCode) {
        const conflicts = [];
        const classMatches = newCode.match(/class\s+(\w+)/g);
        if (classMatches) {
            const classNames = classMatches.map(match => match.replace('class ', '').trim());
     ...
     * 메서드명 충돌 찾기
     */
    findMethodNameConflicts(newCode) {
        const conflicts = [];
     ...
     * 변수명 충돌 찾기
     */
    findVariableNameConflicts(newCode) {
        const conflicts = [];
        const variableMatches = newCode.match(/(\w+)\s*=\s*[^;]+;/g);
        if (variableMatches) {
            const variableNames = variableMatches.map(match => match.split('=')[0]?.trim() || '');
     ...
     * 네임스페이스 충돌 찾기
     */
    findNamespaceConflicts(newCode) {
        const conflicts = [];
        const namespaceMatches = newCode.match(/namespace\s+([^{]+)/g);
        if (namespaceMatches) {
            const namespaces = namespaceMatches.map(match => match.replace('namespace ', '').trim());
     ...
     * 인터페이스 충돌 예측
     */
    async predictInterfaceConflicts(newCode, targetFile) {
        return {
     ...
     * 인터페이스 충돌 찾기
     */
    findInterfaceConflicts(newCode) {
        const conflicts = [];
        const interfaceMatches = newCode.match(/interface\s+(\w+)/g);
        if (interfaceMatches) {
            const interfaceNames = interfaceMatches.map(match => match.replace('interface ', '').trim());
     ...
     * 메서드 시그니처 충돌 찾기
     */
    findMethodSignatureConflicts(newCode) {
     ...
     * 의존성 충돌 예측
     */
    async predictDependencyConflicts(newCode, targetFile) {
        return {
     ...
     * Using 문 충돌 찾기
     */
    findUsingConflicts(newCode) {
        const conflicts = [];
        const usingMatches = newCode.match(/using\s+([^;]+);/g);
        if (usingMatches) {
            const usings = usingMatches.map(match => match.replace('using ', '').replace(';', '').trim());
     ...
     * 로직 충돌 예측
     */
    async predictLogicConflicts(newCode, targetFile) {
        return {
     ...
     * 위험도 계산
     */
    calculateRiskLevel(newCode, targetFile) {
        let riskScore = 0;
        // 네이밍 충돌 점수
     ...
     * 권장사항 생성
     */
    async generateRecommendations(newCode, targetFile) {
...
//# sourceMappingURL=conflictPredictor.js.map