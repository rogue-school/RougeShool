...
export class FileAnalyzer {
  ...
  public async analyzeFile(filePath: string): Promise<any> {
    ...
    try {
      ...
      logger.error(`파일 분석 실패: ${filePath}`, error as Error);
   ...
   * 의존성 분석
   */
  private analyzeDependencies(content: string): any {
    const usingMatches = content.match(/using\s+([^;]+);/g);
    const dependencies = usingMatches ? usingMatches.map(match => 
      match.replace('using ', '').replace(';', '').trim()
    ) : [];

    return {
      external: dependencies.filter(dep => 
        !dep.includes('System') || dep.includes('System.')
      ),
      internal: dependencies.filter(dep => 
        dep.includes('System') && !dep.includes('System.')
      ),
      unity: dependencies.filter(dep => 
        dep.includes('Unity') || dep.includes('Zenject') || dep.includes('DG.Tweening')
      ),
   ...
   * 패턴 분석
   */
  private analyzePatterns(content: string, filePath: string): any {
    const patterns = {
   ...
   * 인터페이스 분석
   */
  private analyzeInterfaces(content: string): any {
    const interfaceMatches = content.match(/interface\s+(\w+)/g);
    const interfaces = interfaceMatches ? interfaceMatches.map(match => 
   ...
   * 클래스 분석
   */
  private analyzeClasses(content: string): any {
    const classMatches = content.match(/class\s+(\w+)/g);
    const classes = classMatches ? classMatches.map(match => 
   ...
   * 품질 분석
   */
  private analyzeQuality(content: string): any {
    return {
   ...
   * 들여쓰기 스타일 감지
   */
  private detectIndentationStyle(lines: string[]): string {
    let spaces = 0;
    let tabs = 0;
  ...
  private calculateAverageMethodLength(content: string): number {
    ...
    const totalLines = methodMatches.reduce((sum, match) => 
      sum + match.split('\n').length, 0
    );
   ...
   * 평균 매개변수 개수 계산
   */
  private calculateAverageParameterCount(content: string): number {
    const methodMatches = content.match(/\([^)]*\)/g);
    if (!methodMatches) return 0;
   ...
   * 한국어 내용 확인
   */
  private hasKoreanContent(content: string): boolean {
   ...
   * 코드 스타일 분석
   */
  private analyzeCodeStyle(content: string): any {
    return {
   ...
   * 중괄호 스타일 감지
   */
  private detectBraceStyle(content: string): string {
    if (content.includes('{\n')) return 'allman';
    if (content.includes('{ ')) return 'k&r';
    return 'unknown';
  }
}
