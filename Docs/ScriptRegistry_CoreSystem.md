...
| **SettingsPanelController** | `Game.CoreSystem.UI` | `UI/SettingsPanelController.cs` | 설정 패널 UI 제어 및 게임 진행 제어 (이어하기/다시하기/메인메뉴로/게임끄기) | `OnContinueClicked()`, `OnResetProgressClicked()`, `OnGoToMainClicked()`, `OnExitGameClicked()` 등 | 버튼 UI 참조, `IStageManager`, `IItemService`, `PlayerManager`, `IPlayerHandManager` DI 주입 | `[Inject(Optional = true)]`로 `ISceneTransitionManager`, `SettingsManager`, `IStageManager`, `IItemService`, `PlayerManager`, `IPlayerHandManager` 주입 | 인게임 설정창 (이어하기/다시하기/메인메뉴/종료 기능) | ✅ 사용 중 |
