...
| **CardDropService** | `Game.CombatSystem.Service` | `SkillCardSystem/DragDrop/CardDropService.cs` (파일 위치는 SkillCardSystem이지만 네임스페이스는 CombatSystem.Service) | 카드 드래그&드롭 처리 서비스 | `RegisterDropHandler(...)`, `TryDropCard(...)` 등 | 슬롯/카드 참조 | `CombatInstaller.BindFactories`에서 `CardDropService` AsSingle 바인딩 | SkillCardSystem Drag&Drop 경로 | ✅ 사용 중 |
...
| 연결 대상 | 연결 방식 | 데이터 흐름 | 설명 |
|----------|----------|------------|------|
| `EnemyCharacter` | DI 주입 (`ISlotMovementController`) | 페이즈 전환 → `UpdateEnemyCache` 호출 | 페이즈 전환 후 새로운 페이즈의 덱을 사용하도록 캐시 업데이트 |
